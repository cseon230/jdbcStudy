...
INSERT INTO TBL_JIKWI(JIKWI_ID, JIKWI_NAME, MIN_BASICPAY) VALUES(JIKWISEQ.NEXTVAL, '사원', 1600000);
--==>> 1 행 이(가) 삽입되었습니다. * 9
...
-- 직원 전체 출력(사번 정렬)
SELECT * FROM TBL_EMP ORDER BY EMP_ID;

-- 직원 전체 출력(이름 정렬)
SELECT * FROM TBL_EMP ORDER BY EMP_NAME;

-- 직원 전체 출력(부서 정렬)
SELECT * FROM TBL_EMP ORDER BY BUSEO_ID;

-- 직원 전체 출력(직위 정렬)
SELECT * FROM TBL_EMP ORDER BY JIKWI_ID;

-- 직원 전체 출력(급여 내림차순 정렬)
SELECT * FROM TBL_EMP ORDER BY BASICPAY + SUDANG DESC;
...
-- 직원 검색 출력(사번 검색)
SELECT EMP, EMP_NAME AS NAME, SSN, IBSADATE, CITY_ID AS CITY, FROM TBL_EMP WHERE EMP_ID = ???

-- 직원 검색 출력(이름 검색)
SELECT * FROM TBL_EMP WHERE EMP_NAME = ???

-- 직원 검색 출력(부서 검색)
SELECT * FROM TBL_EMP WHERE BUSEO_ID = ???

-- 직원 검색 출력(직위 검색)
SELECT * FROM TBL_EMP WHERE JIKWI_ID = ???


-- 입력
INSERT INTO TBL_EMP (EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, SAL) VALUES (EMPSEQ.NEXTVAL, '정한나', '820506-2425153', '2004-06-07', 6, '016-2424-4242', 3, 9, 1000000, 104000, ????)
;
...
-- CITY_ID 가지고 CITY_NAME 알아내기
SELECT E.EMP_ID, E.EMP_NAME, C.CITY_NAME
FROM TBL_EMP E JOIN TBL_CITY C
ON E.CITY_ID = C.CITY_ID;

-- BUSEO_ID 가지고 BUSEO_NAME 알아내기
SELECT E.EMP_ID, E.EMP_NAME, B.BUSEO_NAME
FROM TBL_EMP E JOIN TBL_BUSEO B
ON E.BUSEO_ID = B.BUSEO_ID;


-- JIKWI_ID 가지고 JIKWI_NAME 알아내기
SELECT E.EMP_ID, E.EMP_NAME, J.JIKWI_NAME
FROM TBL_EMP E JOIN TBL_JIKWI J
...
-- CITY, BUSEO, JIKWI 전부 조인 한 결과..
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_NAME, TEL, BUSEO_NAME AS BUSEO, JIKWI_NAME AS JIKWI, BASICPAY, SUDANG, BASICPAY+SUDANG AS SAL
FROM TBL_EMP E JOIN TBL_CITY C
ON E.CITY_ID = C.CITY_ID
               JOIN TBL_BUSEO B
               ON E.BUSEO_ID = B.BUSEO_ID
...
VALUES(EMPSEQ.NEXTVAL, '이호석', '961112-1234567', TO_DATE('2020-11-11', 'YYYY-MM-DD')
      ...
      , (SELECT JIKWI_ID FROM TBL_JIKWI WHERE JIKWI_NAME='대리'), 5000000, 2000000);
--> 한 줄 구성
...
-- 7. 전체 직원 리스트 조회 쿼리문
--SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG, BASICPAY+SUDANG AS SAL
--FROM TBL_EMP;
...
-- 전화번호 없는 직원들을 위한 쿼리문 수정 (NVL)
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_NAME, NVL(TEL, '번호없음'), BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, PAY FROM EMPVIEW ORDER BY EMP_ID
;
